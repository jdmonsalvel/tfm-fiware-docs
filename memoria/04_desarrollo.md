...
![Figura 4.0. Diagrama de funcionamiento de la plataforma FIWARE GitOps](Imagenes/Diagrama de funcionamiento de plataforma.png)
...
![Figura 4.3. Topología de red AWS — VPC en tres capas con subredes públicas, de aplicación y de datos distribuidas en tres zonas de disponibilidad](Imagenes/Fig 4-3 Topología de red AWS.png)
...
![Figura 4.5. Infraestructura AWS — servicios y aplicaciones desplegados por Terraform en eu-west-1](Imagenes/Diagrama infraestructura de servicios aws.png)
...
Kong se despliega en el *namespace* `provider` mediante el chart oficial `kong/kong` v3.2.0 (Kong 3.9) gestionado por ArgoCD (Sync Wave 3). Opera en modo DB-less con configuración declarativa que activa el plugin `jwt` sobre el servicio `orion-ld-service`. El plugin valida la firma HS256 y la expiración del token antes de enrutar la petición a Orion-LD.

**Tabla 4.14.**
*Escenarios de validación de autenticación Kong/JWT (KPI SE-3)*

| Escenario | Resultado esperado | Mecanismo | Resultado obtenido |
|-----------|-------------------|-----------|-------------------|
| Petición sin header `Authorization` | `401 Unauthorized` | Kong rechaza en pre-autenticación | ✅ 401 |
| Token JWT con firma inválida | `401 Unauthorized` | Kong rechaza verificación de firma | ✅ 401 |
| Token JWT expirado | `401 Unauthorized` | Kong verifica claim `exp` | ✅ 401 |
| Token JWT válido (HS256, iss correcto) | `200 OK` + datos NGSI-LD | Kong reenvía a Orion-LD | ✅ 200 |

*Fuente:* Elaboración propia. Mediciones obtenidas del entorno desplegado en Amazon EKS eu-west-1.

> **Figura 4.15** — Salida de curl mostrando los cuatro escenarios de autenticación: 401 sin token, 401 con token inválido, 401 con token expirado y 200 con token válido.
> *Fuente: Elaboración propia. [pendiente de captura]*
