...
- Diseño de la arquitectura de referencia con el modelo C4
...
**Actividades:**
- Implementación de módulos Terraform para VPC (3 AZs), EKS 1.34 y roles IAM
...
- Desarrollo de Helm values para Orion-LD, Keyrock y Kong
- Configuración de External Secrets Operator con AWS Secrets Manager
- Integración del flujo de autenticación iSHARE
- Configuración de ingress-nginx con TLS (cert-manager + Let's Encrypt)
...
- Recopilación de evidencias (capturas, logs, métricas)
...
La validación del modelo se realiza mediante indicadores clave de rendimiento (KPIs) organizados en cuatro dimensiones:
...
|-----------|-------------|---------|---------------|
| IaC | Terraform | ≥ 1.7 | Estándar industria, framework de módulos propio |
| Orquestación | Kubernetes / EKS | 1.34 | Última versión disponible en AWS EKS |
| GitOps | ArgoCD | 7.x (Helm chart) | CNCF Graduated, UI completa |
| Empaquetado | Helm | ≥ 3.14 | Estándar para charts Kubernetes |
| Context Broker | Orion-LD | 1.x | NGSI-LD v1.6 compliant |
| Identity Mgmt | Keyrock | 8.x | iSHARE compatible |
| PEP Proxy | Kong | 3.x | API Gateway / PEP con plugin JWT |
