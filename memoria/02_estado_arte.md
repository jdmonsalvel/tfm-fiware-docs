...
| Chaves et al. | 2022 | FIWARE, i4Trust | Piloto Data Space sector agroalimentario | Implementación manual, sin reproducibilidad |
| Murillo et al. | 2023 | Terraform, EKS, Helm | IaC para plataformas multi-tenant | Sin componentes FIWARE, sin marcos de confianza |
...
**Pilotos industriales de Data Spaces.** Chaves et al. (2022) documentan la implementación de un Data Space basado en FIWARE para el sector agroalimentario en el marco del proyecto i4Trust, un programa de la Comisión Europea orientado a la creación de Data Spaces sectoriales. Este trabajo aporta evidencia de la viabilidad de FIWARE en un contexto de producción real, con más de 20 participantes y flujos de intercambio de datos verificados con iSHARE. No obstante, la implementación se realizó de forma manual y específica para el contexto del piloto, sin abstracción en patrones reutilizables ni automatización del ciclo de vida mediante GitOps.

**IaC para plataformas cloud-native.** Murillo et al. (2023) proponen un framework de módulos Terraform para el aprovisionamiento de plataformas multi-tenant en AWS EKS, siguiendo patrones similares al framework desarrollado en el presente trabajo: módulos con variables de tipo `map(object)` para múltiples instancias y outputs planos `{ nombre → id }`. Los autores demuestran que este patrón reduce en un 60% el código HCL requerido para desplegar entornos adicionales. Sin embargo, el trabajo se centra exclusivamente en la infraestructura base y no incorpora componentes FIWARE, marcos de confianza ni la capa de GitOps para gestión del ciclo de vida.
...
**Tercera conclusión.** De entre las tecnologías analizadas, FIWARE emerge como la única pila con soporte institucional explícito por parte de la DSBA para la construcción de Data Spaces conformes con los estándares europeos. Su integración oficial con el DSBA TCF, el soporte nativo para NGSI-LD y la compatibilidad con iSHARE mediante Keyrock justifican su selección frente a las alternativas consideradas (Eclipse Ditto, FROST-Server). Los trabajos previos (Llorente et al., 2023; Chaves et al., 2022) confirman la viabilidad de FIWARE en producción pero sin la capa de automatización GitOps que el presente trabajo aporta.
